    ...
    try:
        for i in range(5):
            try:
                ...
                BATCH_NUM = 10
                for i in range(int(pages_needed/BATCH_NUM) + 1):
                    for j in range(i*BATCH_NUM, (i+1)*BATCH_NUM):
                        fetch_pool.spawn(fetch_roomid, cate, j + 1, room_list)
                    fetch_pool.join()
